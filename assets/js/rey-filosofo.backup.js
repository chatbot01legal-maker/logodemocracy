...
const REY_FILOSOFO = {
  ...
  try {
    const response = await fetch('/api/reyfilosofo/process', {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        sessionId: reyFilosofoGetSessionId(),
        message: userText
      })
    });
  ...
  if (!tutorReply) {
    tutorReply = `[Error: no se recibió respuesta del tutor]<br><span style="font-size:0.7rem; opacity:0.6;">Diagnóstico: ${diagInfo || 'sin datos'}</span>`;
  }
