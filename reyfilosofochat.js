/* ═══════════════════════════════════════════════════════
   REYFILOSOFOCHAT.JS — Motor Cognitivo v1.1 (Multiorigen)
   Rey Filósofo — Servicio Cognitivo Transversal
   ═══════════════════════════════════════════════════════

   Esta implementación NO es la aplicación definitiva del Rey Filósofo.
   Es el Motor Cognitivo v1.1: el núcleo mínimo y reutilizable que
...
(function () {
  ...
  const state = {
    sessionId: null,
    activeAsset: null,   // el Activo Cognitivo recibido (legado o cacheado)
  ...
  // ─── Resolución Dinámica de Contexto ────────────────
  function resolveActiveContext() {
    const context = { origin: 'unknown', cognitiveAsset: null, sophiaAudit: null };

    // 1. Extraer contexto de Academia
    if (typeof window !== 'undefined' && window.Academy && typeof window.Academy.getCurrentCognitiveAsset === 'function') {
      const asset = window.Academy.getCurrentCognitiveAsset();
      if (asset) {
        context.origin = 'academia';
        context.cognitiveAsset = asset;
      }
    }

    // 2. Extraer contexto de SOPHIA
    if (typeof window !== 'undefined' && window.SOPHIA && typeof window.SOPHIA.getLastEvaluation === 'function') {
      const audit = window.SOPHIA.getLastEvaluation();
      if (audit) {
        context.origin = context.origin === 'academia' ? 'multiorigen' : 'sophia';
        context.sophiaAudit = audit;
      }
    }
  ...
  function normalizeCognitiveAsset(raw) {
    ...
    // Soporta 'source' directo o 'metadata.originModule'
  ...
  async function sendMessage(rawText) {
    ...
    // Resolver contexto en tiempo real
    const activeContext = resolveActiveContext();

    // Fallback: si no pudimos extraer asset en vivo, usamos el cacheado en state.activeAsset
    if (!activeContext.cognitiveAsset && state.activeAsset) {
      activeContext.cognitiveAsset = state.activeAsset;
      if (activeContext.origin === 'unknown') {
        activeContext.origin = state.activeAsset.source || 'unknown';
      }
    }

    if (!activeContext.cognitiveAsset && !activeContext.sophiaAudit) {
      console.warn('⚠️ ReyFilosofoChat: Se está enviando un mensaje sin Activo Cognitivo ni auditoría de SOPHIA.');
    }
    ...
    const payload = {
      ...
      content: userMessage.content, // Retrocompatibilidad para controladores existentes
      provider_module: activeContext.origin,
      activeAsset: activeContext.cognitiveAsset, // Retrocompatibilidad
      context: activeContext // Nuevo formato unificado Multiorigen
    };
  ...
  const ReyFilosofoChat = {
    ...
    open(cognitiveAsset) {
      if (cognitiveAsset) {
        ...
        try {
          ...
          state.conversation = normalized.conversation ? normalized.conversation.slice() : [];
        } catch (e) {
          console.error('❌ ReyFilosofoChat.open():', e.message);
          return;
        }
      } else {
        // Intento de auto-descubrimiento al abrir sin parámetros
        const autoContext = resolveActiveContext();
        if (autoContext.cognitiveAsset) {
          state.activeAsset = autoContext.cognitiveAsset;
        }
      }
    ...
    _getState() {
      return {
        ...
        liveContext: resolveActiveContext()
      };
    }
  };
